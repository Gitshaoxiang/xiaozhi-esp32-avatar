...
class Effect final : public Drawable {
   private:
    ...
    void drawHeartMark(M5Canvas *spi, uint32_t x, uint32_t y, uint32_t r,
        ...
        spi->fillTriangle(x, y, x - r / 2 - a, y + a, x + r / 2 + a, y + a,
                          color);
        spi->fillTriangle(x, y + (r / 2) + 2 * a, x - r / 2 - a, y + a,
                          x + r / 2 + a, y + a, color);
    }

    void drawSleepMark(M5Canvas *spi, uint32_t x, uint32_t y, uint32_t r,
                       uint16_t color, uint16_t bColor, float offset) {
        float scaled_r = r;
        if (scaled_r <= 0) return;
        uint16_t _color = color;
        if (offset > 0.3) {

            spi->drawLine(x, y, x + scaled_r / 2, y, color);  // 顶线
            spi->drawLine(x + scaled_r / 2, y, x, y + scaled_r / 2,
                          color);  // 斜线
            spi->drawLine(x, y + scaled_r / 2, x + scaled_r / 2,
                          y + scaled_r / 2, color);  // 底线
        }

        if (offset > 0.7) {
            
                spi->drawLine(x + scaled_r, y - scaled_r,
                              x + scaled_r / 2 + scaled_r, y - scaled_r,
                              color);  // 顶线
                spi->drawLine(x + scaled_r / 2 + scaled_r, y - scaled_r,
                              x + scaled_r, y + scaled_r / 2 - scaled_r,
                              color);  // 斜线
                spi->drawLine(x + scaled_r, y + scaled_r / 2 - scaled_r,
                              x + scaled_r / 2 + scaled_r,
                              y + scaled_r / 2 - scaled_r, color);  // 底线
    ...
    void draw(M5Canvas *spi, BoundingRect rect, DrawContext *ctx) override {
        ...
        switch (exp) {
            ...
            case Expression::Sleeping:
                drawSleepMark(spi, 190, 35, 30, primaryColor, bgColor, offset);
                break;
            default:
                // noop
                break;
